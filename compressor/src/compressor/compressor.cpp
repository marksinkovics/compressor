...
Compressor::Compressor(const EncodedData& data)
    : encoded_data_(data)
{
    
}

void Compressor::build_dictionary() noexcept
{
...
void Compressor::build_tree() noexcept
{
...
void Compressor::create_hash_table() noexcept
{
...
void Compressor::print_graph() const noexcept
{
    std::cout << "digraph G {\n";
    std::for_each(_tree->preOrderBegin(), _tree->preOrderEnd(), [](std::shared_ptr<BinaryNode> bin_node) {
        auto node = std::dynamic_pointer_cast<SymbolNode>(bin_node);
...
void Compressor::print_dict() const noexcept
{
